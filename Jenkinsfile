...
node {
    ...
    ws("${gopath}/src/github.com/ONSdigital/dp-dd-file-uploader") {
        stage('Checkout') {
            checkout scm
            sh 'git clean -dfx'
        ...
        stage('Build') {
            sh "GOPATH=${gopath} go build -o build/dp-dd-file-uploader"
        }
        ...
        stage('Image') {
            docker.withRegistry("https://${env.ECR_REPOSITORY_URI}", { ->
                ...
                docker.build('dp-dd-file-uploader', '--no-cache --pull --rm .').push(revision)
            })
        }
        ...
        stage('Bundle') {
            ...
            sh "tar -cvzf dp-dd-file-uploader-${revision}.tar.gz appspec.yml scripts/codedeploy"
            sh "aws s3 cp dp-dd-file-uploader-${revision}.tar.gz s3://${env.S3_REVISIONS_BUCKET}/"
        }

        stage('Deploy') {
            sh sprintf('aws deploy create-deployment %s %s %s,bundleType=tgz,key=%s', [
                '--application-name dp-dd-file-uploader',
                "--deployment-group-name ${env.CODEDEPLOY_DISCOVERY_PUBLISHING_DEPLOYMENT_GROUP}",
                "--s3-location bucket=${env.S3_REVISIONS_BUCKET}",
                "dp-dd-file-uploader-${revision}.tar.gz",
            ])
        }
    }
}
